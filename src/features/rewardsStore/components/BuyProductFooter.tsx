...
function BuyProductFooter({ price, productId }: BuyProductFooterProps) {
  ...
  const agencyNumber = Number(agencyCode);

  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);

  const handleConfirm = () => {
    if (!agencyNumber || !productId) return;

    mutation.mutate(
      { agencyCode: agencyNumber, productId },
      {
        onSuccess: (res) => {
          if (res.IsSuccess) {
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="xs">
        <DialogTitle className="text-center font-bold text-lg">تایید خرید</DialogTitle>
